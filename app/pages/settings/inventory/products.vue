        ...
        <div v-if="state.esServicio !== undefined">
            ...
            <header class="mb-8 pb-4 border-b border-gray-200 dark:border-gray-700">
                <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white">
                    Registro de {{ state.esServicio ? 'Servicio' : 'Producto/Insumo' }} 📦
                </h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Complete los detalles para {{ isEditing ? 'actualizar' : 'crear' }} el elemento.</p>
            </header>
            ...
            <div class="flex items-center gap-6 mb-8 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                ...
                <div class="flex items-center">
                    <input id="esServicio" type="checkbox" v-model="state.esServicio" class="h-5 w-5 text-green-600 dark:text-green-400 focus:ring-green-500 border-gray-300 rounded-md transition duration-150 ease-in-out" />
            ...
            <form @submit.prevent="handleFormSubmit" class="space-y-8">
                <div class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-inner-lg">
                    ...
                    <div v-show="filteredTabs[selectedTabIndex]?.originalIndex === 0">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                            ...
                            <div v-if="!state.esServicio" class="col-span-1">
                                ...
                                <select id="condicion" v-model="state.condicion" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5">
                            ...
                            <div v-if="!state.esServicio" class="col-span-1">
                                ...
                                <select id="tipoInventario" v-model="state.tipoInventario" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5">
                            ...
                            <div v-if="!state.esServicio">
                                ...
                                <input type="text" id="codigoBarra" v-model="state.generalidades.codigoBarra" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5" placeholder="Opcional" />
                            </div>
                            ...
                            <div v-if="!state.esServicio" class="md:col-span-2 lg:col-span-2">
                                ...
                                <textarea id="referencia" v-model="state.generalidades.referencia" rows="3" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5" placeholder="Añade referencias, números de parte, etc."></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-show="filteredTabs[selectedTabIndex]?.originalIndex === 1" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <DxDataGridReusable
                            ...
                            key-expr="id"
                            :delete-method="removeCompatibilidad"
                            :columns="[
                                { dataField: 'marca', caption: 'Marca', allowEditing: true },
                                { dataField: 'modelo', caption: 'Modelo', allowEditing: true },
                                { dataField: 'anio', caption: 'Año', dataType: 'number', width: 80, allowEditing: true },
                                { dataField: 'descripcion', caption: 'Descripción', allowEditing: true },
                            ]"
                        ...
                        <div class="space-y-8">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div class="md:col-span-3 flex flex-wrap gap-6 items-center border-b pb-4 border-gray-100 dark:border-gray-700">
                                    <div class="flex items-center">
                                        <input id="aplicaImpuesto" type="checkbox" v-model="state.precios.aplicaImpuesto"
                                            :disabled="state.condicion === 'Usado' && !state.esServicio"
                                    ...
                                    <div class="flex items-center">
                                        <input id="editarPrecio" type="checkbox" v-model="state.precios.editarPrecio" :disabled="state.condicion === 'Usado' && !state.esServicio" class="h-5 w-5 text-green-600 dark:text-green-400 focus:ring-green-500 border-gray-300 rounded" :class="{ 'opacity-60 cursor-not-allowed': state.condicion === 'Usado' && !state.esServicio }" />
                                ...
                                <div v-if="state.precios.aplicaImpuesto || state.esServicio" class="col-span-1">
                                    ...
                                    <select id="impuesto" v-model="state.precios.impuesto" @change="calculateAllPrices()" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="number"
                                                id="costo1"
                                                v-model.number="state.precios.costo1"
                                                step="0.01"
                                                @input="calculatePrices(1, 'costo')"
                                                :class="['mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5', { 'border-red-500 focus:border-red-500 focus:ring-red-500': errors.costo1 }]"
                                                :disabled="false /* state.condicion === 'Usado' && !state.esServicio */"
                                            />
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                v-model.number="state.precios.porcentajeUtilidad1"
                                                step="0.01"
                                                @input="calculatePrices(1, 'porcentaje')"
                                                :class="['mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5', { 'border-red-500 focus:border-red-500 focus:ring-red-500': errors.porcentajeUtilidad1 }]"
                                            />
                                        ...
                                        <div class="col-span-1 md:col-span-2">
                                            <label for="precioNeto" class="block text-sm font-bold text-gray-800 dark:text-white mb-1">
                                                Precio Neto (Sin Impuesto)
                                            </label>
                                            <input
                                                type="number"
                                                id="precioNeto"
                                                :value="state.precios.precio1"
                                                @input="event => { state.precios.precio1 = parseFloat(event.target.value) || 0; calculatePrices(1, 'precio'); }"
                                                step="0.01"
                                                class="mt-1 block w-full rounded-lg border-gray-400 dark:border-gray-600 bg-gray-100 dark:bg-gray-900/50 dark:text-white shadow-inner sm:text-base p-3 font-semibold"
                                            />
                                        </div>

                                        <div class="col-span-1 md:col-span-2">
                                            <label for="precio1" class="block text-sm font-bold text-gray-800 dark:text-white mb-1">
                                                Precio 1 ({{ state.precios.aplicaImpuesto ? state.precios.impuesto : 'C/S Impuesto' }})
                                            </label>
                                            <input
                                                type="number"
                                                id="precio1"
                                                :value="state.precios.precioConImpuesto1"
                                                @input="handlePrice1Input"
                                                step="0.01"
                                                class="mt-1 block w-full rounded-lg border-green-500 dark:border-green-500 bg-green-100 dark:bg-green-900/50 dark:text-white shadow-inner sm:text-base p-3 font-semibold"
                                            />
                                            <p v-if="state.precios.aplicaImpuesto" class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                *Valor del impuesto incluido: Lps. {{ (state.precios.precioConImpuesto1 - state.precios.precio1).toFixed(2) }}
                                            </p>
                                        </div>
                                ...
                                <div class="mt-6 border-t pt-6 border-gray-200 dark:border-gray-700 space-y-6">
                                    ...
                                    <div v-for="n in [2, 3, 4]" :key="n" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-3 border border-gray-100 dark:border-gray-700 rounded-lg">
                                        <div>
                                            ...
                                            <input type="number" :id="'precio' + n" v-model.number="state.precios['precio' + n]" step="0.01" @input="calculatePrices(n)" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5" />
                                        </div>
                                        <div>
                                            <label :for="'porcentajeUtilidad' + n" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Utilidad %</label>
                                            <input type="number" :id="'porcentajeUtilidad' + n" v-model.number="state.precios['porcentajeUtilidad' + n]" step="0.01" @input="calculatePrices(n, 'porcentaje')" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5" />
                                        </div>
                                        <div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select
                                        id="almacen"
                                        v-model="state.inventario.almacenId"
                                        :class="['mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5', { 'border-red-500 focus:border-red-500 focus:ring-red-500': errors.almacenId }]"
                                    >
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <select id="bin" v-model="state.inventario.binId" :disabled="!state.inventario.almacenId" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5" :class="{ 'opacity-60 cursor-not-allowed': !state.inventario.almacenId }">
                                        ...
                                        <button type="button" @click="toggleBinForm" :disabled="!state.inventario.almacenId" class="mt-1 px-4 py-2 border border-transparent text-lg font-medium rounded-lg shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out" :class="{ 'opacity-50 cursor-not-allowed': !state.inventario.almacenId }">
                                            +
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div v-if="showBinForm" class="mt-6 p-5 border border-dashed border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label for="newBinCodigo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Código de Estantería</label>
                                ...
                                <div class="flex justify-end gap-3 mt-4">
                                    <button type="button" @click="showBinForm = false" class="inline-flex justify-center rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out">
                                        Cancelar
                                    </button>
                                    <button type="button" @click="createBin" class="inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-green-600 text-sm font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out">
                                        Crear Estantería
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div class="md:col-span-1">
                                        ...
                                        <input
                                            type="number"
                                            id="existenciasIniciales"
                                            v-model.number="state.stock.existenciasIniciales"
                                            min="0"
                                            :class="['mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5', { 'border-red-500 focus:border-red-500 focus:ring-red-500': errors.existenciasIniciales }]"
                                        />
                    ...
                    <div v-show="filteredTabs[selectedTabIndex]?.originalIndex === 4">
                        <div class="space-y-4">
                            ...
                            <div class="overflow-x-auto shadow-inner rounded-lg border border-gray-300 dark:border-gray-700">
                                <div id="notes-editor" class="bg-gray-50 dark:bg-gray-900/70 text-gray-900 dark:text-white rounded-lg p-4 min-h-[200px] focus:ring-green-500 focus:border-green-500 outline-none" contenteditable="true" @input="updateNotes" v-html="state.notas"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-show="filteredTabs[selectedTabIndex]?.originalIndex === 5">
                        <div class="space-y-6">
                            ...
                            <div v-if="!state.imagen" class="p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-center hover:border-green-400 dark:hover:border-green-500 transition duration-150 ease-in-out">
                                <label class="cursor-pointer text-sm font-medium text-gray-700 dark:text-gray-300">
                                    ...
                                    <span class="text-green-600 dark:text-green-400 hover:text-green-700 font-semibold transition duration-150 ease-in-out">haz clic para seleccionar</span>
                                </label>
                            </div>
                            ...
                            <div v-else class="mt-4 flex flex-wrap gap-4">
                                <div class="relative w-48 h-48 group">
                                    ...
                                    <button @click="removeImage" type="button" class="absolute top-0 right-0 bg-red-500 text-white rounded-full p-1 text-xs -mt-2 -mr-2 flex items-center justify-center h-8 w-8 hover:bg-red-600 transition duration-150 ease-in-out shadow-lg">
                ...
                <div class="mt-8 pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                    <button type="button" @click="$emit('cancel')" class="inline-flex items-center px-6 py-3 border border-gray-300 dark:border-gray-600 text-base font-medium rounded-lg shadow-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 focus:ring-offset-gray-50 dark:focus:ring-offset-gray-900 transition duration-150 ease-in-out">
                        Cancelar
                    </button>

                    <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg shadow-xl text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-offset-2 focus:ring-green-500 focus:ring-offset-gray-50 dark:focus:ring-offset-gray-900 transition duration-150 ease-in-out">
...
// ⚠️ Asegúrate que la ruta sea correcta. Si el archivo es 'useProductForm.js', quita la 's' de 'useProductsForm'
...
// Eventos que notificarán al componente padre (DataGrid)
const emit = defineEmits(['save-success', 'cancel']);

// Importar la Store de Pinia
const store = useProductStore();

// Importar todas las funciones y estados del Composable
const {
    state,
    errors, // 🔑 AGREGADO: Objeto de errores
    selectedTabIndex,
    showBinForm,
    newBin,
    ...
    handleImageUpload,
    removeImage,
    // 🔑 RENOMBRAMOS la función de envío para que solo obtenga datos del composable
    onSubmit: getFormData,
    resetState, // 🔑 AGREGADO: Función para resetear el estado
} = useProductForm(props.initialData);

// 🚀 FUNCIÓN WRAPPER PRINCIPAL PARA EL SUBMIT Y LLAMADA A PINIA
const handleFormSubmit = async () => {
    console.log("Formulario: Iniciando proceso de guardado...");

    // 1. Obtener los datos limpios del composable. El composable ahora valida internamente.
    const productData = getFormData();

    // 2. Si la validación falló en el composable (retorna null), se detiene
    if (!productData) {
        console.warn("Formulario: Datos no válidos, deteniendo el guardado.");
        return;
    }

    try {
        // 3. Ejecutar la acción de guardado en Pinia.
        const savedProduct = await store.saveOrUpdateProduct(productData, props.isEditing);

        // 4. Si el guardado fue exitoso (Pinia devuelve el objeto guardado)
        if (savedProduct) {
            console.log("Formulario: Guardado exitoso. Notificando al padre.");
            // 5. Emitir el evento de éxito para que el DataGrid se cierre y sepa que hubo un cambio
            emit('save-success', savedProduct);

            // 🔑 6. LIMPIAR EL FORMULARIO después del guardado exitoso
            if (!props.isEditing) {
                resetState();
                console.log("Formulario: Estado reseteado para nuevo ingreso.");
            }
        }
    } catch (error) {
        console.error("Error al guardar el producto/servicio:", error);
        // Utiliza una notificación adecuada si tienes DevExtreme o similar
