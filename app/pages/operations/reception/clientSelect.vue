<template>
  <div class="main-layout">
    <div class="progress-bar-container">
      <template v-for="step in steps" :key="'step-'+step.id">
        <div class="progress-step" :class="{ active: currentStep >= step.id }">
          <div class="step-icon">{{ step.id }}</div>
          <div class="step-label">{{ step.label }}</div>
        </div>
        <div v-if="step.id < steps.length" :key="'line-'+step.id" class="progress-line" :class="{ active: currentStep >= step.id + 1 }"></div>
      </template>
    </div>
...
.progress-step.active .step-icon { background-color: #2563eb; color: white; }
